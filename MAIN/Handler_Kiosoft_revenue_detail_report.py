...
def process_df(df):
    ...
    logger.debug(f'Filtered DataFrame to only include APPROVAL responses. {filtered_df.shape[0]} rows remain.')
    ...
    subtotal_df = sorted_df.groupby('Machine ID').agg({'Total Amount ($)': 'sum'}).reset_index()
